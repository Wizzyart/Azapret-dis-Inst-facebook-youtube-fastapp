...
if "%~1"=="load_game_filter" (
    ...
    for /f "tokens=1,2,3,4 delims=|" %%A in ("!GameFilterStatus!^|!GameFilter!^|!GameFilterTCP!^|!GameFilterUDP!") do (
        endlocal
        set "GameFilterStatus=%%~A"
        set "GameFilter=%%~B"
        set "GameFilterTCP=%%~C"
        set "GameFilterUDP=%%~D"
    )
...
if "%~1"=="admin_choice" (
    ...
    call :load_user_lists
    call :game_switch_status
    set "AUTO_MENU_CHOICE=%~2"
    set "AUTO_EXIT=1"
    echo Started with admin rights
    if "%~2"=="1" goto service_install
    if "%~2"=="2" goto service_remove
    if "%~2"=="3" goto service_status
    if "%~2"=="4" goto game_switch
    if "%~2"=="5" goto ipset_switch
    if "%~2"=="6" goto check_updates_switch
    if "%~2"=="7" goto ipset_update
    if "%~2"=="8" goto hosts_update
    if "%~2"=="9" goto service_check_updates
    if "%~2"=="10" goto service_diagnostics
    if "%~2"=="11" goto run_tests
    exit /b 1
)
...
if defined AUTO_MENU_CHOICE (
    set "menu_choice=%AUTO_MENU_CHOICE%"
) else (
...
:: Searching for bypass .bat files in the bypasses folder, with root fallback
echo Pick one of the options:
set "count=0"
for /f "delims=" %%F in ('powershell -NoProfile -Command "$base=(Get-Location).Path; $dir=Join-Path $base 'bypasses'; if (Test-Path -LiteralPath $dir) { $files=Get-ChildItem -LiteralPath $dir -Filter 'general*.bat' } else { $files=Get-ChildItem -LiteralPath $base -Filter 'general*.bat' }; $files | Sort-Object { [Regex]::Replace($_.Name, '(\d+)', { $args[0].Value.PadLeft(8, '0') }) } | ForEach-Object { $_.FullName.Substring($base.Length + 1) }"') do (
    set /a count+=1
    echo !count!. %%F
    set "file!count!=%%F"
)
...
chcp 437 > nul
cls
echo Update check is disabled in this build.
if "%1"=="soft" exit
pause
goto menu
...
if not exist "%gameFlagFile%" (
    ...
    set "GameFilter=1024"
    set "GameFilterTCP=1024"
    set "GameFilterUDP=1024"
    exit /b
)
